...
def create_user(username, password, nombre, apellidos, email, role, dept_id, manager_id, comunidad, horas, dias_vac):
    c = _conn()
    c.execute("""INSERT INTO users (username,password_hash,nombre,apellidos,email,role,department_id,manager_id,comunidad_autonoma,horas_semanales,dias_vacaciones_anuales)
                 VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
...
def create_vac_request(user_id, f_ini, f_fin, tipo, comentario, comunidad="madrid"):
    ...
    c = _conn()
    c.execute("""INSERT INTO vacation_requests (user_id,fecha_inicio,fecha_fin,tipo,dias_laborables,comentario_empleado)
                 VALUES (?,?,?,?,?,?)""",
